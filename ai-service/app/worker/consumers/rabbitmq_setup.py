...
def setup_feedback_queue(channel):
    channel.exchange_declare(
        exchange=AI_DIRECT_EXCHANGE, exchange_type="direct", durable=True
    )
    channel.exchange_declare(
        exchange=AI_DLX_EXCHANGE, exchange_type="direct", durable=True
    )
    ...
    channel.queue_declare(
        queue=FEEDBACK_REQUEST_QUEUE, durable=True, arguments=QUEUE_ARGUMENTS
    )
    channel.queue_bind(
