...
export default function Dashboard() {
  ...
  const loadStats = useCallback(async () => {
    ...
    // Fire all requests concurrently to minimize perceived loading time
    // 并行加载所有数据，缩短用户感知的加载时间
    ...
    // Derive recommended jobs from score history, deduplicated by jobId
    // 从评分历史组装推荐职位列表，按 jobId 去重仅保留最新记录
    if (scoreHistoryResult.status === 'fulfilled' && jobsResult.status === 'fulfilled' && resumesResult.status === 'fulfilled') {
      ...
      // Keep only the most recent score per job since history is sorted by createdAt desc
      // history 已按 createdAt 降序，取每个职位第一条即为最新评分
