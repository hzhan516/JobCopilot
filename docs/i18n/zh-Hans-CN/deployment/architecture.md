# JobCopilot — Docker Compose 部署架构

> [English](../../../deployment/architecture.md) | [繁體中文](../../../i18n/zh-Hant-TW/deployment/architecture.md)

## 1. 概述

JobCopilot 是一个以**三层 Docker 网络架构**部署的 AI 驱动求职平台：面向公网的反向代理层、内部应用层以及隔离的数据库层。
...
# 2. 编辑 .env，替换所有 [replace-me] 占位符
vim .env

# 3. 生成强 JWT 密钥（48 字节 = 64 个 base64 字符）
openssl rand -base64 48
# 将输出粘贴到 .env 的 JWT_SECRET 中

# 4. 生成内部 API 密钥（32 字节 = 44 个 base64 字符）
