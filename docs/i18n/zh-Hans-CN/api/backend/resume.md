...
- 不要使用 `groupId` 作为下载参数
- 后端已通过文档转换器支持导出格式转换。支持的目标格式为 `pdf`、`docx`、`html`、`md` 和 `txt`。
...
|------|-----|
| **接口名称** | 创建版本副本 |
| **接口路径** | `POST /api/v1/resumes/groups/{groupId}/versions` |
| **是否需要认证** | 是 |
| **Content-Type** | `application/json` |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `sourceVersionId` | String (UUID) | 否 | 源版本ID；为空时，从当前 ACTIVE CONVERTED 版本复制 |

**版本链上限**: 每个简历组最多保留 50 个 CONVERTED 版本（含 ARCHIVED）。超出上限时，系统会在创建新副本前自动删除最旧的 ARCHIVED 版本。

**自动归档**: 创建新的 CONVERTED 版本时，同一组内现有的 ACTIVE CONVERTED 版本会自动归档。
...
| 创建版本副本 | POST | `/api/v1/resumes/groups/{groupId}/versions` | 创建版本副本 | 是 |
