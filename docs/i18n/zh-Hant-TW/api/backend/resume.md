...
**自動向量重生成**: 編輯 `CONVERTED` 或 `AI_OPTIMIZED` 類型的版本時，系統在內容持久化後會自動觸發異步向量重生成請求，確保後續職位匹配召回使用最新的履歷嵌入向量。若 MQ 訊息發送失敗，編輯操作仍會成功返回；向量將在下次可用時機自動補齊。
...
|------|-----|
| **介面名稱** | 建立版本副本 |
| **介面路徑** | `POST /api/v1/resumes/groups/{groupId}/versions` |
| **是否需要認證** | 是 |
| **Content-Type** | `application/json` |
...
| 欄位 | 型別 | 必填 | 說明 |
|------|------|------|------|
| `sourceVersionId` | String (UUID) | 否 | 來源版本 ID；為空時，從目前 ACTIVE CONVERTED 版本複製 |

**版本鏈上限**: 每個履歷組最多保留 50 個 CONVERTED 版本（含 ARCHIVED）。超出上限時，系統會在建立新副本前自動刪除最舊的 ARCHIVED 版本。

**自動歸檔**: 建立新的 CONVERTED 版本時，同一組內現有的 ACTIVE CONVERTED 版本會自動歸檔。
...
| 建立版本副本 | POST | `/api/v1/resumes/groups/{groupId}/versions` | 建立版本副本 | 是 |
