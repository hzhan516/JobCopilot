...
| `llmOverallScore` | Float | 目前配置的 LLM 適配度評估器返回的整體分數 |
| `semanticMatch` | Float | 選用，pgvector 召回得到的語義相似度分數 |
| `datasetScore` | Float | 選用，自適應資料集模型產生的分數 |
| `finalScore` | Float | 最終融合得分（0.0-1.0） |
| `llmModel` | String | 選用，LLM 適配度評估使用的模型識別 |
