...
| **內網（Internal）** | `backend`、`ai-service`、`ai-worker`、`rabbitmq`、`redis`、`minio` | 無（僅 Docker DNS） | 服務間透過容器名稱通信 |
...
| **職責** | 後端、AI Service 與 AI Worker 之間的非同步訊息代理（Outbox 模式，訊息佇列）。 |
...
| **安全說明** | 無外部存取。專供 `ai-worker` (寫入) 和 `ai-service` (讀取) 使用。 |
