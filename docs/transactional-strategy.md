...
| 7 | **Aggregate roots must have optimistic locking (`@Version`)** | Prevent lost updates in concurrent write scenarios. Currently applied to `Job`, `ResumeGroup`, `Conversation`, `JobMatchResult`, `ApplicationTracking`, `User`. |
